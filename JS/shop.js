  ...
  function displayProducts(products) {
    ...
    products.forEach(function(product) {
      ...
      const shortDescriptionText = shortDescription ? shortDescription : "During the mid-1980s, the rap group NWA rises from the harsh streets of Compton, located in Los Angeles, California. Their music and lyrics, which depict life in the hood, lead to a revolution in Hip Hop culture.";
      const shortDescriptionUrl = shortDescription ? shortDescription.url : "";
      const shortDescriptionDisplay = `<p>${shortDescription}</p>`;

     
  
      // Update the HTML template to include a link to the product details page with the id of the selected product
      html += `
        <div class="product">
          <h2>${product.name}</h2>
...
function displayProducts(products) {
  ...
  products.forEach(function(product) {
    ...
    const formattedPrice = prefix + price.toLocaleString(undefined, { minimumFractionDigits: product.prices.currency_minor_unit, maximumFractionDigits: product.prices.currency_minor_unit }) + suffix;
